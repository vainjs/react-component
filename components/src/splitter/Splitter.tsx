import type { InitHandleState, SplitterProps, SplitterRef, PanelDataRef, ResizeEvent, PanelSizes, Panels } from './type'
import {
  type ReactNode,
  useImperativeHandle,
  useCallback,
...
import { getPrevAndNextResultByHandle, isMouseEvent, getMovement } from './utils'
...
const Splitter = forwardRef<SplitterRef, SplitterProps>((props, ref) => {
  ...
  const registerResizeHandle = useCallback(
    (handId: string) => {
      return (e: ResizeEvent, handle: HTMLDivElement) => {
        if (!activeHandleId || activeHandleId !== handId || !splitterRef.current) return
        e.preventDefault()
        const movement = getMovement(e, handle, layout, initHandleStateRef.current.correctOffset)
        if (movement === 0) return
        const result = getPrevAndNextResultByHandle(handle, splitterRef.current, movement, layout, panels)
        if (!result) return
  ...
  const registerPanel = useCallback((panelId: string, panelRef: PanelDataRef) => {
  ...
  const onStartDragging = useCallback((handId: string, event: ResizeEvent) => {
    ...
    if (isMouseEvent(event)) {
      ...
      initHandleStateRef.current = {
        correctOffset: event.clientX - resizeHandle.getBoundingClientRect().left,
      }
    }
  }, [])
      ...
      >
        {Children.toArray(children).reduce(
          (acc: ReactNode[], child, index, array) =>
            [...acc, child, index !== array.length - 1 && <ResizeHandle />].filter(Boolean),
          []
        )}
      </div>
